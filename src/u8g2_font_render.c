...
void U8G2FontRender_PrintChar(U8G2FontRender_t *font, uint8_t *x, uint8_t y, char chr) {
	uint16_t search_position = U8G2FontRender_GetStartSymbolSearchPostition(font, chr);

	U8G2FontGlyph_t glyph;
	if (U8G2FontRender_GetGlyph(font, &glyph, search_position, chr) != U8G2FontRender_OK) {
		return;
	}
	U8G2FontRender_ParseGlyphHeader(font, &glyph);

//	U8G2FontRender_RenderGlyph(font, &glyph, *x, y);
...
void U8G2FontRender_RenderGlyphBuf(U8G2FontRender_t *font, U8G2FontGlyph_t *glyph, uint8_t x, uint8_t y, uint16_t fg_color, uint16_t bg_color, uint16_t *buf) {
  ...
  while (1) {
    ...
    for (; repeat>=0; repeat--) {
      for (uint8_t i=0; i<zeros + ones; i++) {
        offset = (x_pos + font->header.glyph_x_offset) + (y_pos - font->header.glyph_y_offset) * font->header.bounding_box_width - 1;
        if (i <= zeros - 1) {
          memcpy(buf + offset, &bg_color, 2);
        } else {
          memcpy(buf + offset, &fg_color, 2);
        }
        x_pos++;
